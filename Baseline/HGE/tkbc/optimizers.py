...
class IKBCOptimizer(object):
    ...
    def epoch(self, examples: torch.LongTensor):
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            b_begin = 0
            while b_begin < examples.shape[0]:
                time_range = actual_examples[b_begin:b_begin + self.batch_size].to(self.device)

                ## RHS Prediction loss
                sampled_time = (
                        torch.rand(time_range.shape[0]).to(self.device) * (time_range[:, 4] - time_range[:, 3]).float() +
                        time_range[:, 3].float()
                ).round().long()
                ...
                if self.model.has_time():
                    ...
                    truth = (
                            torch.rand(these_examples.shape[0]).to(self.device) * (these_examples[:, 4] - these_examples[:, 3]).float() +
                            these_examples[:, 3].float()
                    ).round().long()
                    time_predictions = self.model.forward_over_time(these_examples[:, :3].to(self.device).long())
                    time_loss = loss(time_predictions, truth.to(self.device))
