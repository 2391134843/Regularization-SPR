...
def prepare_dataset(path, name):
    ...
    json.dump(relations_to_id, open('tkbc/instance_test/GDELT_rid.json', 'w'), indent=2)
    timestamps_to_id = {x: i for (i, x) in enumerate(sorted(timestamps))}
    json.dump(timestamps_to_id, open('tkbc/instance_test/GDELT_tid.json', 'w'), indent=2)
    json.dump(entities_to_id, open('tkbc/instance_test/GDELT_eid.json', 'w'), indent=2)
...
if __name__ == "__main__":
    #datasets = ['GDELT']
    # datasets = ['ICEWS14','GDELT', 'icews14', 'icews05-15']
    datasets = ['ICEWS14','GDELT', 'icews05-15']
    for d in datasets:
